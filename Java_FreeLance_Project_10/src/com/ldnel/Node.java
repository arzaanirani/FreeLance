...
public class Node implements Drawable{
    ...
    public final static int SMALL_NODE_RADIUS = 10;
    ...
    private ArrayList<Edge> edges = new ArrayList<Edge>(); //the node's edges
    ...
    public void setLabel(String aLabel){
        if(aLabel != null && aLabel.trim().length()> 0)
            label = aLabel.trim();
    }
    ...
    public static Node parseFromFile(BufferedReader graphXMLDataStream) {
        ...
        */
        ...
        Node parsedNode = new Node(0, 0);  //location to be replaced
        ...
        try {
            ...
            while ((inputLine = graphXMLDataStream.readLine()) != null) {
                ...
                if (dataString.startsWith(Node.endTag)) return parsedNode;
                else if(dataString.startsWith(Node.IDStartTag) &&
                        dataString.endsWith(Node.IDEndTag)){
                        String theIDString = dataString.substring(
                                Node.IDStartTag.length(),
                                dataString.length() - Node.IDEndTag.length()
                        );
                        parsedNode.setID(Integer.parseInt(theIDString));
                }
                else if(dataString.startsWith(Node.labelStartTag) &&
                        dataString.endsWith(Node.labelEndTag)){
                        String theLabelString = dataString.substring(
                            Node.labelStartTag.length(),
                            dataString.length() - Node.labelEndTag.length()
                    );
                    parsedNode.setLabel(theLabelString.trim());
                }
                else if(dataString.startsWith(Node.locationStartTag) &&
                        dataString.endsWith(Node.locationEndTag)){
                    String theLocationString = dataString.substring(
                            Node.locationStartTag.length(),
                            dataString.length() - Node.locationEndTag.length()
                    );
                ...
                else if(dataString.startsWith(Node.selectedTag) ){
